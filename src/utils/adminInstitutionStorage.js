...
export async function createInstitution(values) {
  return rpc("admin_create_institution", {
    p_name: values.name.trim(),
    p_access_code: values.access_code.trim().toUpperCase(),
    p_student_limit: Number(values.student_limit),
    p_expires_at: values.expires_at,
    p_status: values.status,
    p_notes: values.observations?.trim() || null,
  });
}

export async function updateInstitution(id, values) {
  return rpc("admin_update_institution", {
    p_id: id,
    p_name: values.name.trim(),
    p_access_code: values.access_code.trim().toUpperCase(),
    p_student_limit: Number(values.student_limit),
...
export async function listInstitutionStudents(institutionId) {
  const data = await rpc("admin_list_institution_students", {
    p_institution_id: institutionId,
  });
  return Array.isArray(data) ? data : [];
}

export async function createInstitutionStudent(institutionId, values) {
  return rpc("admin_create_institution_student", {
    p_institution_id: institutionId,
    p_full_name: values.full_name.trim(),
    p_student_code: values.student_code.trim().toUpperCase(),
    p_pin: values.pin,
    p_grade: Number(values.grade),
    p_avatar: values.avatar || "/assets/avatar-1.png",
    p_status: values.status || "active",
  });
}

export async function updateInstitutionStudent(id, values) {
  return rpc("admin_update_institution_student", {
    p_id: id,
    p_full_name: values.full_name.trim(),
    p_student_code: values.student_code.trim().toUpperCase(),
    p_grade: Number(values.grade),
    p_avatar: values.avatar,
    p_status: values.status,
  });
}

export async function resetInstitutionStudentPin(id, pin) {
  await rpc("admin_reset_institution_student_pin", {
    p_id: id,
    p_pin: pin,
  });
}
